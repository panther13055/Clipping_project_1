...
# ---------- app.js: keep high-resolution exports on a real media clock
p = Path("app.js")
s = p.read_text(encoding="utf-8")

s = rep(s, '''  const EXPORT_PROFILES = {
...
s = rep(s, '''  function drawFrame(bg, revealed, ui, pos) {
...
s = rep(s, '''    let frameIdx = 0, timer = null;
    ...
    startTimer();''', '''    let frameIdx = 0, timer = null;
    ...
    document.addEventListener("visibilitychange", visibilityHandler);
    encodeFrame(true);
    startTimer();''', "WebCodecs media clock")
...
s = rep(s, '''      async stop() {
        ...
        if (encErr) throw encErr;''', '''      async stop() {
        ...
        if (encErr) throw encErr;''', "WebCodecs A/V duration lock")

s = rep(s, '    const stream = exportCanvas.captureStream(30);', '    const stream = exportCanvas.captureStream(profile.fps || 30);', "MediaRecorder fps")
s = rep(s,
...
# ---------- local_ai_server.py: portable Real-ESRGAN + Chromium localhost access
p = Path("local_ai_server.py")
s = p.read_text(encoding="utf-8")
s = rep(s,
...
s = rep(s, '''        model = realesrgan_model(content)
        ...
        if tier == "max" and os.environ.get("LOCAL_AI_TTA", "0") == "1":
            cmd.append("-x")
        run(cmd, cwd=exe_dir)''', "Real-ESRGAN portable model path")
s = rep(s, '''        self.send_header("Access-Control-Allow-Headers", "Content-Type, X-Filename")
        self.send_header("Access-Control-Expose-Headers", "X-Enhancer-Engine")''', '''        self.send_header("Access-Control-Allow-Headers", "Content-Type, X-Filename")
        # Required by current Chromium when an HTTPS site talks to localhost
        # after the user grants Local Network Access.
        self.send_header("Access-Control-Allow-Private-Network", "true")
        self.send_header("Access-Control-Expose-Headers", "X-Enhancer-Engine, X-Processing-Seconds")''', "Local Network Access CORS")
p.write_text(s, encoding="utf-8")


# ---------- local_ai_bridge.js: actionable diagnostics
p = Path("local_ai_bridge.js")
s = p.read_text(encoding="utf-8")
s = rep(s, '''      const neural = !!d.realesrgan;
      const msg = neural
      ...
      return d;''', '''      const neural = !!d.realesrgan;
      ...
      return d;''', "Local AI health message")
s = rep(s,
