...
s = s.replace('"8K":    { width: 4320, height: 7680, videoBitrate: 85_000_000, codec: "avc1.64003e", fps: 24, renderFps: 24 }',
              '"8K":    { width: 4320, height: 7680, videoBitrate: 52_000_000, codec: "avc1.64003e", fps: 24, renderFps: 24 }')
s = s.replace('"4K":    { width: 2160, height: 3840, videoBitrate: 38_000_000, codec: "avc1.640033", fps: 30, renderFps: 30 }',
              '"4K":    { width: 2160, height: 3840, videoBitrate: 30_000_000, codec: "avc1.640033", fps: 30, renderFps: 30 }')
s = s.replace('"2K":    { width: 1440, height: 2560, videoBitrate: 20_000_000, codec: "avc1.640032", fps: 30, renderFps: 30 }',
              '"2K":    { width: 1440, height: 2560, videoBitrate: 18_000_000, codec: "avc1.640032", fps: 30, renderFps: 30 }')

start = s.index('  async function exportVideo() {')
end = s.index('\n  // ---------------------------------------------- save / load a whole project', start)

new_block = r'''  function recorderFormatChoice() {
    const candidates = [
